...
export function TeacherDashboard() {
  ...
  useEffect(() => {
    api
      .get<TeacherDashboardData>("/teacher/dashboard")
      .then(setData)
      .catch(() => {});
  }, []);
  ...
  return (
    <div className="space-y-6 md:space-y-8">
      <div>
        ...
        <h1 className="text-2xl md:text-3xl font-display font-semibold text-ink leading-tight max-w-2xl">
          Which students need help, and why?
        </h1>
      </div>
        ...
        <div className="card p-4 md:p-5 min-w-0">
          ...
          <div className="w-full h-[240px] min-w-0">
            {data.engagement.length === 0 ? (
              <div className="h-full flex items-center justify-center text-sm text-slate text-center">
                No quiz activity in the last 7 days.
              </div>
            ) : (
              <ResponsiveContainer width="100%" height="100%">
                <LineChart
...
function StatCard({
  ...
  return (
    <div className="card p-3 md:p-4 min-w-0">
      ...
      <p className={`font-mono text-xl md:text-2xl font-semibold ${color}`}>
        {value}
      </p>
