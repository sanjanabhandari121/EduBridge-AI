...
export function Sidebar({ role }: { role: "STUDENT" | "TEACHER" }) {
  ...
  return (
    <aside className="w-60 shrink-0 border-r border-black/5 bg-white flex flex-col h-screen sticky top-0">
          ...
          <NavLink
            ...
            className={({ isActive }) =>
              `flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-colors ${
                isActive ? "bg-teal-light text-teal-dark" : "text-slate hover:bg-paper-dim hover:text-ink"
              }`
            }
          >
